...
def create_fc_layer(input,
    ...
    layer = tf.nn.dropout(layer, keep_prob=0.85)
    if use_relu:
        layer = tf.nn.relu(layer)
    return layer
...
layer_fc3 = create_fc_layer(input=layer_fc2,
                            num_inputs=layer_fc2.get_shape()[1:4].num_elements(),
                            num_outputs=fc_layer_size3,
                            use_relu=True)
layer_fc4 = create_fc_layer(input=layer_fc3,
                            num_inputs=fc_layer_size3,
...
def train(num_iteration):
    ...
    for i in range(total_iterations, total_iterations + num_iteration):
        ...
        if i % 1000 == 0:
            ...
            saver.save(session, '/media/data2/mhy/models/model1/SimpleFusion.ckpt', global_step=i)
    total_iterations += num_iteration


train(num_iteration=200001)